    ...
    <div className="min-h-screen bg-[#f4f5f7] text-ink">
      <header className="fixed inset-x-0 top-0 z-30 border-b border-white/70 bg-white/60 backdrop-blur-2xl">
        <div className="mx-auto flex h-[74px] max-w-[1320px] items-center justify-between px-6">
          <a href="#server-demo" className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-ink text-white"><Cube size={19}/></span><div><p className="text-[14px] font-semibold tracking-[.08em]">AVEON NETWORK</p><p className="text-[10px] text-muted">Minecraft 1.21</p></div></a>
      ...
      <main>
        ...
        <section id="store" className="mx-auto max-w-[1320px] scroll-mt-24 px-6 py-24">
          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{filtered.map((p,i)=><article key={p.id} className="glass group flex min-h-[330px] flex-col rounded-[30px] p-6 transition duration-700 hover:-translate-y-1"><div className="flex items-start justify-between"><span className="glass-circle flex h-14 w-14 items-center justify-center rounded-2xl"><LogoMark size={26}/></span>{(p.popular||i===1)&&<span className="rounded-full bg-ink px-3 py-1 text-[10px] uppercase tracking-[.08em] text-white">Популярное</span>}</div><p className="mt-8 text-[10.5px] uppercase tracking-[.12em] text-muted">{p.category}</p><h3 className="mt-2 text-[22px] font-medium">{p.name}</h3><p className="mt-2 text-[12.5px] leading-relaxed text-muted">{p.description}</p><div className="mt-auto flex items-center justify-between pt-7"><b className="text-[23px] font-semibold">{p.price} ₽</b><button onClick={()=>setSelected(p)} className="btn-black flex h-10 items-center gap-2 rounded-full px-4 text-[12px]">Купить <ArrowRight size={13}/></button></div></article>)}</div>
        </section>
      ...
      {selected && <div className="fixed inset-0 z-50 flex items-center justify-center p-5"><button onClick={()=>setSelected(null)} className="absolute inset-0 bg-ink/25 backdrop-blur-xl"/><div className="glass relative w-full max-w-md rounded-[32px] p-8"><button onClick={()=>setSelected(null)} className="absolute right-6 top-6 text-xl text-muted">×</button><p className="eyebrow">Оформление покупки</p><h3 className="mt-5 text-[28px] font-medium">{selected.name}</h3><p className="mt-2 text-sm text-muted">{selected.description}</p><form onSubmit={buy} className="mt-7"><label className="text-[12px] font-medium">Никнейм игрока<input required name="player" pattern="[A-Za-z0-9_]{3,16}" placeholder="Steve" className="mt-2 h-12 w-full rounded-2xl border border-ink/10 bg-white/60 px-4 text-sm outline-none focus:border-ink/30"/></label><div className="mt-7 flex items-center justify-between"><b className="text-[25px]">{selected.price} ₽</b><button className="btn-black h-12 rounded-full px-6 text-[13px]">Оплатить картой</button></div><p className="mt-5 text-[10.5px] leading-relaxed text-muted">Демонстрационный платёж. Денежные средства не списываются.</p></form></div></div>}
      {success && <div className="fixed inset-0 z-50 flex items-center justify-center p-5"><button onClick={()=>setSuccess(null)} className="absolute inset-0 bg-ink/25 backdrop-blur-xl"/><div className="glass relative max-w-sm rounded-[34px] p-9 text-center"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-ink text-white"><Check size={27}/></span><h3 className="mt-6 text-[26px] font-medium">Покупка оплачена</h3><p className="mt-3 text-sm leading-relaxed text-muted">Товар выдан игроку <b className="font-medium text-ink">{success.player}</b>. Платёж {success.id} появился в панели.</p><button onClick={()=>setSuccess(null)} className="glass-pill mt-7 h-11 rounded-full px-6 text-sm">Продолжить</button></div></div>}
    </div>
  );
}